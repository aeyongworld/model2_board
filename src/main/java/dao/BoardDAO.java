...
//
//
//    public int getNext(int pageNumber) { // 페이징 처리를 위해 다음에 작성될 게시물의 board_id 반환
//        int offset = (pageNumber - 1) * 10;
//        String SQL = "SELECT  board_id  FROM board ORDER BY board_id DESC LIMIT 1 OFFSET ?";
//        try {
//            PreparedStatement pstmt = conn.prepareStatement(SQL);
...
//
//        try {
//            int lastBoardId = getNext(pageNumber);
//
//            PreparedStatement pstmt = conn.prepareStatement(SQL);
//            pstmt.setInt(1, lastBoardId);
//            rs = pstmt.executeQuery();
//
//            while (rs.next()) {
